...
from api.services import DemoError, MAX_BODY_BYTES, OwnershipError, SessionStore, Settings, StripeGateway, belongs_to, checkout_params, payment_dto, require_uuid, _value
...
def config(response: Response):
    ...
    return {"configured": config.configured, "test_only": config.test_key, "partner_configured": config.partner_configured, "partner_ready": None, "partner_checked": False, "currency": "eur"}
...
async def create_session(request: Request, response: Response):
    ...
    response.set_cookie("partner_demo_session", token, httponly=True, secure=True, samesite="lax", max_age=3600, path="/")
    no_store(response)
...
async def checkout(request: Request, partner_demo_session: str | None = Cookie(default=None)):
    ...
    try:
        ...
        if not stripe_gateway.verify_partner(config.partner_account_id):
            raise HTTPException(503, "Partner payment configuration is unavailable.")
        created = stripe_gateway.create_checkout(checkout_params(config, session["id"], data.get("amount_cents"), data.get("fee_percent")), scoped_key)
...
async def webhook(request: Request, stripe_signature: str | None = Header(default=None)):
    ...
    if len(raw) > MAX_BODY_BYTES:
        raise HTTPException(413, "Request is too large.")
    try:
        event = stripe.Webhook.construct_event(raw, stripe_signature, config.stripe_webhook_secret, tolerance=300)
    ...
    if _value(event, "type") in {"checkout.session.completed", "payment_intent.succeeded", "charge.refunded"}:
        obj = _value(_value(event, "data", {}), "object", {})
        metadata = _value(obj, "metadata", {}) or {}
        demo_session_id = _value(metadata, "demo_session_id")
        pi_id = _value(obj, "payment_intent") if _value(event, "type") != "payment_intent.succeeded" else _value(obj, "id")
